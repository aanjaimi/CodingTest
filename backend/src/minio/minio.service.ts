...
export class MinioService extends Client {
  ...
  async uploadFile(username: string, file: Express.Multer.File) {
    console.log(file);
    // const filename = this.generateRandomFilename(file.originalname);
    // const objectName = path.join(username, filename);
  ...
  // async removeFile(username: string, file: Express.Multer.File) {
  //   const objectName = path.join(username, file.filename);
